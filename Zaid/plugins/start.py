...
async def start(event):
     if event.is_group:
       await event.reply("**Register now!**",buttons=[
        [Button.url("Register Here 🤍", f"http://www.9987up.cc/#/register?r_code=355725271")]])
       return
...
async def start(event):
     if event.is_group:
       await event.reply("**I know you want to earn money, Come and join**",buttons=[
        [Button.url("Check it out", f"https://www.tclottery7.com/")]])
       return
...
async def start(event):
     if event.is_group:
       await event.reply("**Contact Us Now**",buttons=[
        [Button.url("Go to Channel ➡ ", f"https://t.me/+EG3mh-vS9Y82N2Rl")]])
       return
